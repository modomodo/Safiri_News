...
public class NewsRecyclerAdapter extends RecyclerView.Adapter<NewsRecyclerAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        //Get the current news article fomr the array list listNews for the current position in the recyclerview
        News currentNews = mListNews.get(position);
        //Use the holder to set the text for the Title
        urlPhoto = currentNews.getUrlImg();
        loadImages(urlPhoto,holder);
        holder.title.setText(currentNews.getTitle());
        CharSequence relativeTime = DateUtils.getRelativeTimeSpanString(currentNews.getTimestamp()); //Get the relative time from the timestamp retrieved in the JSON feed
        holder.domain.setText(currentNews.getDomain());
